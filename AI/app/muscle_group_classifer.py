...
def predict_muscle_group(data_to_predict):
    data_to_predict = prepare_data_to_predict(data_to_predict)
    model=load_model("muscle_group")
    result=model.predict(data_to_predict)
    result=add_muscle_groups_label(result[0])
    print("workout_value is",result)
    return result

def train_muscle_group_and_evaluate(training_dataset,testing_dataset,random_forest_gridsearch,support_vector_machine_gridsearch):
    print("trained!")
    # return 92.0
    ...
    save_model_as_pickle(best_model_to_store, "muscle_group_"+best_model)

    return (accuracy*100) , best_model


def predicate_muscle_group_classifier(data_to_predict):
    """
    Assuming that dataset has been stored and is ready to be loaded
    """

    data_to_predict=Preprocess_data(data_to_predict,features_to_split)
    data_to_predict, _ = Return_X_y(data_to_predict,['label','muscle group','image','Unnamed: 0'])
    loaded_model = pickle.load(open("muscle_group_model.pkl", "rb"))
